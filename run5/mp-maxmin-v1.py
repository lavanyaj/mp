...
random.seed(241)
mpmath.dps = 100
zero53 = mpmath.mpf(str("".join(['0']*100)))
################################################################
...
class MPMaxMin:
    def __init__(self, routes, c):
        ...
        self.t = 0                                            
        # sending rates
        self.x = np.zeros((self.num_flows,1), dtype=mpmath.mpf)        
        self.old_demands = np.zeros((self.num_flows,1), dtype=mpmath.mpf)
        ...
        self.link_to_flow = np.zeros((self.num_links, self.num_flows), dtype=mpmath.mpf)
    ...
    def water_filling(self): 
        weights = np.ones((self.num_flows, 1), dtype=mpmath.mpf)
        x = np.zeros((self.num_flows,1), dtype=mpmath.mpf)
        ...
        while rem_flows.size != 0:
            ...
            cap_bl = rem_cap[bl][0]
            if (type(cap_bl) == int):
                cap_bl = mpmath.mpmathify(cap_bl)
            lw_bl = link_weights[bl][0]
            if (type(lw_bl) == int or type(lw_bl) == float):
                lw_bl = mpmath.mpmathify(lw_bl)

            assert(type(cap_bl) == mpmath.mpf)
            assert(type(lw_bl) == mpmath.mpf)
            inc = cap_bl/lw_bl  #rem_cap[bl]/link_weights[bl]
            x[rem_flows] = x[rem_flows] + inc*weights[rem_flows]

            rem_cap = rem_cap - inc*link_weights
            if (not (rem_cap >= zero53).all()):
                neg = np.where(rem_cap < zero53)
                print "links with neg. rem_cap ", rem_cap[neg]
            ...
            print ", inc was ", inc, "adj.. was ", adj_link_weights[bl]/adj_rem_cap[bl]
...
def gen_random_instance(nswitches, nflows_per_link, safe=True):
    ...
    if (safe):
        A = np.zeros((nflows+nlinks, nlinks), dtype=mpmath.mpf)
        for j in range(nlinks):
            flow_index = nflows+j
            A[flow_index, j] += 1.0
    else:
        A = np.zeros((nflows, nlinks), dtype=mpmath.mpf)
    ...
    c = np.ones((num_used_links, 1), dtype=mpmath.mpf)
    return A,c
