...
class MPMaxMin:
    ...
    def water_filling(self): 
        ...
        while rem_flows.size != 0:
            ...
            adj_link_weights = np.where(link_weights>zero53, link_weights, minusone) # avoid divide by zero
            adj_rem_cap = np.where(rem_cap>zero53, rem_cap, minusone)
            assert((adj_rem_cap<=self.c).all())            
            bl = np.argmax(np.where(adj_link_weights>zero53, adj_link_weights/adj_rem_cap, minusone))
            assert(link_weights[bl] > zero53)            
            #fs = list(np.where(adj_link_weights>zero53, adj_link_weights/adj_rem_cap, float('-1')))
            #print "fair shares in level ", level, ":", fs[:5], "..", fs[-5:]
            ...
            print "level ", level, " bottleneck link is ", bl, ", bottleneck flow has rate ", x[bf[0]]
