...
class MPMaxMin:
    def __init__(self, routes, c):
        ...
        self.t = 0                                            
        # sending rates
        self.x = np.zeros((self.num_flows,1), dtype=np.longdouble)        
        self.old_demands = np.zeros((self.num_flows,1), dtype=np.longdouble)
        ...
        self.link_to_flow = np.zeros((self.num_links, self.num_flows), dtype=np.longdouble)
    ...
    def water_filling(self): 
        weights = np.ones((self.num_flows, 1), dtype=np.longdouble)
        x = np.zeros((self.num_flows,1), dtype=np.longdouble)
    ...
    def update_link_to_flow(self):
        new_rate = np.zeros((self.num_links,1), dtype=np.longdouble)
...
def gen_random_instance(nswitches, nflows_per_link, safe=True):
    ...
    if (safe):
        A = np.zeros((nflows+nlinks, nlinks), dtype=np.longdouble)
        for j in range(nlinks):
            flow_index = nflows+j
            A[flow_index, j] = 1
    else:
        A = np.zeros((nflows, nlinks), dtype=np.longdouble)
