...
class MPMaxMin:
    def __init__(self, routes, c):
        ...
        # max-min rates
        self.maxmin_level = -1
        self.maxmin_x = self.water_filling() #mpmath.zeros(self.num_flows,1)
        # iteration
        self.t = 0                                            
        # sending rates
    ...
    def water_filling(self): 
        ...
        while rem_flows.size != 0:
            ...
            if len(rem_bf) > 0:
                print " bottleneck flow has rate "
                mpmath.nprint(x[rem_bf[0]], 200)
            else:
                print "\n"
    ...
    def update_link_to_flow(self):
        ...
        for l in range(self.num_links):
            #print "\nlink " + str(l)
            flow_indices = []
            old_rate = self.old_rate[l,0]
            sumsat = 0
            numsat = 0
            maxsat = 0
            nflows = 0
            for f in range(self.num_flows):
                if self.routes[f, l] > zero53:
            ...
            new_rate[l,0] = self.c[l,0] - sumsat + maxsat
            assert(numsat < nflows)
            if (numsat < nflows):
                new_rate[l,0] = (self.c[l,0]-sumsat)/(nflows - numsat)
                #print "new_rate " + str(new_rate[l])
            #else:
            #print "new_rate " + str(new_rate[l])

            if new_rate[l,0] < maxsat:
                bottleneck_level = maxsat
            else:
                bottleneck_level = new_rate[l,0]
                 
            #print "bottleneck_level " + str(bottleneck_level)
            for f in flow_indices:
                self.link_to_flow[l, f] = bottleneck_level
...
#         self.old_num_unsat = new_num_unsat
    ...
    def update_flow_to_link(self):
        for f in range(self.num_flows):
            link_indices = []            
            min_rate = mpmath.mpf('100')
            for l in range(self.num_links):
                if self.routes[f, l] > zero53:
                    link_indices.append(l)
                    if self.link_to_flow[l,f] < min_rate:
                        min_rate = self.link_to_flow[l,f]
            self.x[f,0] = min_rate
            l1 = link_indices[0]
            for l in link_indices:                
                self.old_demands[f,0] = self.flow_to_link[f, l1]
                self.flow_to_link[f,l] = self.x[f,0]
